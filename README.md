...
```bash
git clone https://github.com/bailuo-xisi/xianyu-auto-reply.git
cd xianyu-auto-reply
bash deploy.sh
```

- 首次运行会自动生成 `.env` 配置文件和 `docker-compose.deploy.yml`
- 从最新源码构建 Docker 镜像并启动
...
该脚本**不内置 mysql/redis 容器**，仅构建并启动 4 个应用服务（frontend / backend-web / websocket / scheduler），
数据库连接信息通过 `.env.remote` 配置。与方式一相同，直接下载脚本执行即可：
...
# 3) 再次运行：校验配置 → 自动生成 docker-compose.remote.yml → 本地构建 → 启动
curl -fsSL https://raw.githubusercontent.com/bailuo-xisi/xianyu-auto-reply/main/deploy_remote.sh | sed 's/\r$//' | bash
```
...
| `scripts/Pipeline脚本-xianyu-auto-reply.groovy` | Jenkins | 旧版 CI/CD 流水线；当前推荐使用 GitHub Actions + 服务器源码构建 |
